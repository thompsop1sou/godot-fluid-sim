...
# The droplet meshes
var cs_droplet_mesh: Mesh = preload("res://cs_droplet/cs_droplet_mesh.tres")
var cpp_droplet_mesh: Mesh = preload("res://cpp_droplet/cpp_droplet_mesh.tres")
...
func _ready() -> void:
	...
	PhysicsServer3D.shape_set_data(droplet_body_shape, 0.167)
...
func _generate_droplet_scene(server_type: DropletServerType)  -> void:
	...
	var droplet_position := Vector3(randf_range(-1.0, 1.0),
									randf_range(-1.0, 1.0),
									randf_range(-1.0, 1.0))
	droplet_node.position = droplet_position

# Generates a droplet as an RID
func _generate_droplet_rid(server_type: DropletServerType) -> void:
	...
	if server_type == DropletServerType.CS_SERVER:
		droplet_mesh = cs_droplet_mesh
	elif server_type == DropletServerType.CPP_SERVER:
		droplet_mesh = cpp_droplet_mesh
	...
	PhysicsServer3D.body_add_shape(droplet_body_rid, droplet_body_shape)
	...
	PhysicsServer3D.body_set_force_integration_callback(droplet_body_rid, _droplet_moved, num_droplets)
	if server_type == DropletServerType.CS_SERVER:
		CsDropletServerAutoload.add_droplet(droplet_body_rid)
	elif server_type == DropletServerType.CPP_SERVER:
